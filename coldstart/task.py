...
def train(net: Net, trainloader: DataLoader, epochs: int, lr: float, device: torch.device) -> float:
    net.to(device)
...
def test(net: Net, testloader: DataLoader, device: torch.device) -> tuple[float, int, int, int, int, np.ndarray, np.ndarray]:
