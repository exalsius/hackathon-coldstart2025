...
TARGETS = dict(A=0.45, B=0.31, C=0.19, D=0.04)
SPLIT_RATIOS = (0.8, 0.1, 0.1)  # train/eval/test (patient-wise)
OUTDIR = "xray_fl_datasets"
...
def score_patient(row, silo):
    ...
    if silo == "A":
        ...
        if view == "PA": score -= 10000.0

        # Strong age preference for elderly
        if age >= 60: score += 8.0
        elif age < 40: score -= 100.0
        else: score += (age - 40) / 5.0  # gradual increase 40-60

        # Gender bias: more males
        if gender == "M": score += 5.0

        for lb, w in PREF_A.items():
            if lb in labs: score += w

    elif silo == "B":
        # Outpatient PA clinic: PA, younger females, nodules/mass
        if view == "AP": score -= 10000.0

        # Strong age preference for working age
        if 20 <= age <= 65: score += 10.0

        # Gender bias: more females
        if gender == "F": score += 5.0
...
def report_split_stats(name, patient_ids, df_images, ptab):
    ...
    print(f"{name}")
    print(f"{'='*60}")
    print(f"Patients: {n_patients:,} | Images: {n_images:,} ({n_images/n_patients:.1f} img/patient)")
...
def main():
    ...
    for silo in ["A", "B", "C"]:
        ...
        train_p, temp = train_test_split(silo_pids, test_size=0.2, random_state=RNG_SEED)
        eval_p, test_p = train_test_split(temp, test_size=0.5, random_state=RNG_SEED)

        # Create datasets for each split
        for split_type, pids in [("train", train_p), ("eval", eval_p), ("test", test_p)]:
            ...
            print(f"  Created split {silo}_{split_type}: {len(pids)} patients, {len(splits[f'{silo}_{split_type}'])} images")
    ...
    print(f"  Created split D_test: {len(d_pids)} patients, {len(splits['D_test'])} images")
    ...
    # Hospital A
    hospital_a = DatasetDict({
        "train": splits["A_train"],
        "eval": splits["A_eval"]
    })
    hospital_a.save_to_disk(os.path.join(OUTDIR, "HospitalA"))
    print(f"✓ HospitalA saved: train={len(hospital_a['train'])}, eval={len(hospital_a['eval'])}")

    # Hospital B
    hospital_b = DatasetDict({
        "train": splits["B_train"],
        "eval": splits["B_eval"]
    })
    hospital_b.save_to_disk(os.path.join(OUTDIR, "HospitalB"))
    print(f"✓ HospitalB saved: train={len(hospital_b['train'])}, eval={len(hospital_b['eval'])}")

    # Hospital C
    hospital_c = DatasetDict({
        "train": splits["C_train"],
        "eval": splits["C_eval"]
    })
    hospital_c.save_to_disk(os.path.join(OUTDIR, "HospitalC"))
    print(f"✓ HospitalC saved: train={len(hospital_c['train'])}, eval={len(hospital_c['eval'])}")
    ...
    print(f"✓ Test saved: test_A={len(test['test_A'])}, test_B={len(test['test_B'])}, test_C={len(test['test_C'])}, test_D={len(test['test_D'])}")
